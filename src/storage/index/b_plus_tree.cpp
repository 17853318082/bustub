...
auto BPLUSTREE_TYPE::FindLeaf(const KeyType &key, Operation operation, Transaction *transaction, bool leftMost,
  ...
  while (!node->IsLeafPage()) {
    ...
    auto child_node = reinterpret_cast<BPlusTreePage *>(
        child_page->GetData());  // 这里应该先将页面转换为，然后判断该节点是不是叶子页
...
auto BPLUSTREE_TYPE::Split(N *node) -> N * {
  ...
  if (node->IsLeafPage()) {
    ...
    new_leaf->Init(page->GetPageId(), node->GetParentPageId(), leaf_max_size_);  //  初始化叶子节点
...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *transaction) {
  // 对根节点进行加锁
  root_page_id_latch_.WLock();
  transaction->AddIntoPageSet(nullptr);  // nullptr 代表解锁根节点
  // 查看树是否为空
  if (IsEmpty()) {
    ReleaseLatchFromQueue(transaction);
    return;
...
auto BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction) -> bool {
  ...
  auto *parent_node = reinterpret_cast<InternalPage *>(parent_page->GetData());
  auto index = parent_node->ValueIndex(node->GetPageId());  // 查找该节点在父节点中的位置
...
void BPLUSTREE_TYPE::Redistribute(N *neighbor_node, N *node,
  ...
  if (node->IsLeafPage()) {
    ...
    if (from_prev) {
      neighbor_leaf_node->MoveLastToFrontOf(leaf_node);  // 从后面拿一个放在当前节点的第一个
      parent->SetKeyAt(index, leaf_node->KeyAt(0));      // 替换掉当前父节点的key值
    } else {
      neighbor_leaf_node->MoveFirstToEndOf(leaf_node);  // 从兄弟节点的第一个拿到当前节点
