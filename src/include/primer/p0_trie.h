  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (size_t i = 0; i < key.size(); i++) {
      ...
      if (cur_node->get()->HasChild(key[i])) {
        ...
        // 将其插入到子节点中
        cur_node = cur_node->get()->InsertChildNode(key[i],
                                                    std::make_unique<TrieNode>(key[i]));  // 插入一个新节点,并进入子节点
      }
      ...
      if (i == key.size() - 1) {
        ...
        auto new_node = new TrieNodeWithValue(std::move(**cur_node),
                                              value);  // 如果是最后一个则构造一个有值节点，将值存入节点，加入到树中
